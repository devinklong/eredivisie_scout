...
All three data sources (Transfermarkt, soccerdata, WhoScored) fully extracted, verified, and loaded into Postgres. Extraction phase complete. Entity resolution across all three sources is fully complete: FBref, WhoScored, and Transfermarkt are all matched and linked via `player_source_crosswalk` (1,785 canonical players, see `docs/v1_roadmap.md`). One real identity-merge bug (two different real people silently matched as one) found and fixed along the way, plus a systemic audit confirming it was isolated. Model training not yet started.
...
- Transfermarkt fees/market value — **done, loaded into Postgres.** All 29 clubs that played Eredivisie since 2010-11, filtered to real Eredivisie seasons: `eredivisie_transfers` (8,060 rows), `eredivisie_club_status` (288 rows, the season-participation reference table). Market value *history* (not just current snapshot) and injury history not yet built.
- Player bio (height, preferred foot) — **done, loaded into Postgres (2026-09-07/08).** `eredivisie_transfermarkt_player_bio`, keyed on Transfermarkt's own numeric player_id. Transfermarkt does NOT track weight at all -- confirmed via a real profile page, not assumed. 22.3% of players have neither field, confirmed via a season-by-season breakdown to be a real historical data gap (much sparser pre-2017), not a scraper bug -- see `docs/patch_list.md`.

### v1.5 (future)

- ~~Composite~~ **Duel engagement stats (scoped 2026-09-13, re-run pending)** — Opta/WhoScored has no single event type matching the colloquial sense of "duel." It's assembled from several separate winner/loser event-type pairs across categories (`Aerial` win/loss — confirmed self-reporting via real data, see `docs/whoscored_qualifier_taxonomy.md` — `Tackle` vs. `Dispossessed`/unsuccessful `TakeOn`, `Challenge` [always a loss] vs. the opponent's `TakeOn` [the win]). **Decided NOT to blend into one composite number** — kept as separate `ws_ground_duel_win_pct`/`ws_aerial_duel_win_pct` features instead, since a blended rate would wash out position-specific skill profiles (a center-back's aerial game vs. a winger's ground game) that matter for both modeling and building player archetypes. `pipelines/whoscored/derive_aerial_stats.py` built and confirmed; the 13-season re-run to backfill aerial data is scripted but not yet run.
...
- **Transfermarkt** — **done, loaded into Postgres.** All 29 clubs that played Eredivisie since 2010-11: full multi-season transfer history (fees, categorized by type: permanent/loan/free/undisclosed/historical-unknown) plus player bio (height, preferred foot -- NOT weight, confirmed unavailable from this source). Filtered against a manually-compiled season-participation reference table so only genuine Eredivisie-season transfers are counted. **Plain `requests` works for the bulk transfer-history pages, but NOT for individual player profile pages** -- confirmed the hard way (every profile-page request returned 405 Not Allowed, including a manual retest of a URL already working in a real browser). Profile scraping needed the same SeleniumBase UC-mode Cloudflare-bypass approach as FBref/WhoScored -- see `docs/patch_list.md` for the full debugging trail. Not yet built: market value *history* (only current snapshot so far), injury history, loan buy-option/obligation-to-buy terms.
